<?php
class EmbalagensTiposModel extends Connection {
    const TABLE = 'tb_embalagens_tipos';
    private $conn;
    ...
    private $fields = array(
        'id_embalagens_tipos'=>array('type'=>'integer', 'requered'=>true, 'max'=>10, 'key'=>true, 'description'=>'ID'),
    ...
    public function loadId($id) {
        try {
            ...
            $sql = "select p.*
                      from ".self::TABLE." p
                     where p.id_embalagens_tipos = :ID";
    ...
    public function add($arr) {
        try {
            ...
            if (isset($values[':ID_EMBALAGENS_TIPOS'])) {
                unset($values[':ID_EMBALAGENS_TIPOS']);
            }
    ...
    public function del($id){
        try {
            $save = $this->conn->update(
                self::TABLE, 
                array(':STATUS'=>'D'), array(':ID_EMBALAGENS_TIPOS'=>$id)
            );
            return $save;
        } catch (Exception $e) {
            return $e->getMessage();
