<?php
class ComputadoresModel extends Connection {
    const TABLE = 'tb_computadores';
    private $conn;
    ...
    public function loadIdUsuarios($id_usuarios='', $status='') {
        try {
            ...
            if (!empty($id_usuarios)) {
                ...
                $and .= " and u.id_usuarios = :ID_USUARIOS";
            }
            
            $sql = "select p.*
                      from ".self::TABLE." p
                      inner join tb_computadores_usuarios u on u.id_computadores = p.id
                     where 1 = 1                        
                       ".$and."";
            $res = $this->conn->select($sql, $arr);
