<?php
class MateriaisMarcasModel extends Connection {
    const TABLE = 'tb_materiais_marcas';
    private $conn;
    ...
    private $fields = array(
        'id_materiais_marcas'=>array('type'=>'integer', 'requered'=>true, 'max'=>10, 'key'=>true, 'description'=>'ID'),
    ...
    public function loadId($id) {
        try {
            ...
            $sql = "select p.*
                      from ".self::TABLE." p
                     where p.id_materiais_marcas = :ID";
    ...
    public function add($arr) {
        try {
            ...
            if (isset($values[':ID_MATERIAIS_MARCAS'])) {
                unset($values[':ID_MATERIAIS_MARCAS']);
            }
    ...
    public function del($id){
        try {
            $save = $this->conn->update(
                self::TABLE, 
                array(':STATUS'=>'D'), array(':ID_MATERIAIS_MARCAS'=>$id)
            );
            return $save;
        } catch (Exception $e) {
            return $e->getMessage();
