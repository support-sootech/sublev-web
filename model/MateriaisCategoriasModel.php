<?php
class MateriaisCategoriasModel extends Connection {
    const TABLE = 'tb_materiais_categorias';
    private $conn = false;
    ...
    private $fields = array(
        'id_materiais_categorias'=>array('type'=>'integer', 'requered'=>true, 'max'=>10, 'key'=>true, 'description'=>'ID'),
    ...
    public function add($arr) {
        try {
            ...
            if (isset($values[':ID_MATERIAIS_CATEGORIAS'])) {
                unset($values[':ID_MATERIAIS_CATEGORIAS']);
            }
    ...
    public function del($id){
        try {
            $save = $this->conn->update(
                self::TABLE, 
                array(':STATUS'=>'D'), array(':ID_MATERIAIS_CATEGORIAS'=>$id)
            );
            return $save;
        } catch (Exception $e) {
            return $e->getMessage();
